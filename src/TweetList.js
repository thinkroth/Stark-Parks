thinkroth.views.TweetList = Ext.extend(Ext.Panel, {
    hashtag: '',
    layout: 'fit',
    initComponent: function(){
        ...
        thinkroth.views.TweetList.superclass.initComponent.call(this);
    },
    ...
    selectTweet: function(sel, records){
        if (records[0]) {
            Ext.Msg.confirm('External Link', 'Open tweet in Twitter?', function(res){
                if (res === 'yes') {
                    window.location = 'http://twitter.com/' + records[0].data.from_user;
                }
...
Ext.reg('tweetlist', thinkroth.views.TweetList);